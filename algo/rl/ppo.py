...
class PPOAgent(BaseAgent):
    ...
    def __init__(self, configs):
        ...
        self.actor = StochasticActor(self.state_dim, configs['actor_hidden_size'], self.action_dim, state_std_independent=True).to(self.device)
    ...
    def update(self, state, action, next_state, reward, done):
        ...
        with torch.no_grad():
            Rs, advantages = self.gae(self.critic, states, rewards, not_dones, next_states)
            
            action_mean, action_std = self.actor(states)
            fixed_action_distribution = Normal(action_mean, action_std)
            fixed_log_action_probs = fixed_action_distribution.log_prob(actions).sum(axis=-1, keepdims=True)
        
        # update actor and critic
        full_idx = np.array(list(range(self.replay_buffer.size)))
        for _ in range(self.update_times):
            np.random.shuffle(full_idx)
            for idx in np.split(full_idx, self.replay_buffer.size // self.mini_batch_size):
                # critic loss
                ...
                log_action_probs = action_distribution.log_prob(actions[idx]).sum(axis=-1, keepdims=True)
                
                ratio = torch.exp(log_action_probs - fixed_log_action_probs[idx])
                surr1 = ratio * advantages[idx]
                surr2 = torch.clamp(ratio, 1.-self.epsilon_clip, 1.+self.epsilon_clip) * advantages[idx]
