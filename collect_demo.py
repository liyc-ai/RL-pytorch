...
def _collect_demo(
    ...
    for _ in tqdm(range(cfg.demo.n_traj), desc="Collecting demo"):
    ...
    save_dataset_to_h5(dataset, tracking.log_dir, "expert_demo")

    return (
        f"Successfully save expert demonstration into {tracking.log_dir}/expert_demo.hdf5!",
        dataset,
    )
...
def main(cfg: DictConfig):
    ...
    tracking.print(load_torch_model(agent.models, cfg.expert_model_path))

    # collect expert dataset
    tracking.print(f"Collecting expert data on the environment {cfg.env.id}...")
    tracking.print(_collect_demo(agent, env, reset_env_fn, cfg, tracking)[0])
