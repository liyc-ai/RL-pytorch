...
class SimpleReplayBuffer:
    ...
    def add(self, state, action, next_state, reward, done):
        ...
        self.not_done_buffer[self.ptr] = torch.as_tensor(1. - done).to(self.device)
    ...
    def clear(self):
        self.state_buffer = torch.zeros((self.buffer_size, self.state_dim), dtype=torch.float32).to(self.device)
        self.action_buffer = torch.zeros((self.buffer_size, self.action_dim), dtype=torch.float32).to(self.device)
        self.next_state_buffer = torch.zeros((self.buffer_size, self.state_dim), dtype=torch.float32).to(self.device)
        self.reward_buffer = torch.zeros((self.buffer_size, 1), dtype=torch.float32).to(self.device)
