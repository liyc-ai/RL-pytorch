...
from RLA.easy_log.log_tools import Filter, PrettyPlotterTool
from RLA.rla_argparser import boolean_flag
from smart_logger.front_page.page import start_page_server


def argsparser():
    parser = argparse.ArgumentParser("Pretty Plotter")
    # reduce setting
    ...
    parser.add_argument(
        ...
        "-wks",
        type=str,
        default="./.pretty_plotter_cache",
        help="Path to the workspace, used to saving cache data",
    )
    parser.add_argument("--user_name", "-u", type=str, default="user", help="user name")
    parser.add_argument(
        "--password", "-pw", type=str, default="123456", help="password"
    )
...
if __name__ == "__main__":
    ...
    tool = PrettyPlotterTool(
        proj_root=args.data_root, task_table_name=args.task_table_name, regex=args.regex
    )
    tool.gen_json(args.regex)
    if args.start_server:
        plot_config.DATA_PATH = os.path.abspath(args.data_root)
        page_config.WORKSPAPCE = os.path.abspath(
            os.path.expanduser(args.workspace_path)
        )

        plot_config.DATA_MERGER = []
        plot_config.PLOTTING_XY = []
        plot_config.PLOT_LOG_PATH = f"{plot_config.DATA_PATH}"
        plot_config.PLOT_FIGURE_SAVING_PATH = (
            f"{os.path.join(os.path.dirname(plot_config.DATA_PATH), 'figure')}"
        )
